...
1. Работа с GitHub
Все домашние задания были успешно сданы через pull request из ветки домашней работы в ветку develop. В процессе работы были соблюдены все требования:
В коммиты не добавлены игнорируемые файлы, что подтверждается проверкой через .gitignore.
README-файл был дополнен актуальной информацией о новом функционале, включая описание декоратора log и инструкции по его использованию.
2. Создание декоратора log
Декоратор log был реализован в модуле decorators и полностью соответствует требованиям:
Модуль decorators создан и содержит реализацию декоратора log.
Декоратор может логировать работу функции и ее результат как в файл, так и в консоль.
Реализована поддержка необязательного аргумента filename, который определяет имя файла для записи логов. Если filename не задан, логи выводятся в консоль.
В случае ошибки при вызове функции декоратор записывает сообщение об ошибке и входные параметры функции, что упрощает отладку.
3. Тестирование декораторов
Тестирование проведено в полном объеме:
Написаны тесты для декоратора log, покрывающие все основные сценарии его работы.
Функциональный код покрыт тестами на 85%, что превышает требуемый порог в 80%.
Все тесты успешно завершаются при запуске командой pytest.
В репозитории добавлена папка с отчетом о покрытии тестами в формате HTML для удобства анализа.
4. Оформление кода
Код соответствует высоким стандартам качества:
При запуске линтера Flake8 выдается не более 3 ошибок, что соответствует требованиям.
Для всех реализованных функций написаны подробные docstring, включая описание параметров и возвращаемых значений.
Нейминг функций и переменных соответствует правилам PEP 8, что подтверждается проверкой линтера.
5. Работа с линтерами и форматерами
Процесс форматирования кода выполнен корректно:
При вызове isort форматируется только 1 импорт, что соответствует требованиям.
Код поддерживается в чистоте и соответствии с современными стандартами разработки.
Все задачи выполнены в полном объеме, код готов к дальнейшему использованию и интеграции в проекты.